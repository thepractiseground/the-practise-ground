...
export async function generateStaticParams() {
  return MATHS_GRADES.map((g) => ({ grade: String(g) }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Grade ${grade} Maths Quiz - ${totalQ}+ Free Questions`,
    description: `Free Maths practice quizzes for Grade ${grade} students. ${weeks.length} weekly quizzes with ${totalQ}+ questions covering ${MATHS_GRADE_INFO[grade]?.topics || "key maths topics"}.`,
    keywords: [
      `grade ${grade} maths quiz`, `class ${grade} maths questions`,
      `maths practice grade ${grade}`, `CBSE maths class ${grade}`,
      `maths MCQ grade ${grade}`, `free maths quiz class ${grade}`,
    ],
    alternates: { canonical: `https://www.thepractiseground.in/quiz/maths/${grade}` },
  };
}

export default async function MathsGradePage({ params }: Props) {
  const { grade: gradeStr } = await params;
  const grade = parseInt(gradeStr);
  ...
  return (
    <div className="bg-theme-gradient min-h-screen">
        ...
        {weeks.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 max-w-md mx-auto">
              We are preparing Grade {grade} Maths quizzes. Check back soon for weekly practice questions!
            </p>
            <Link href="/quiz/maths" className="inline-block mt-6 bg-emerald-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-emerald-600 transition-colors">
              Browse Other Grades
            </Link>
          </div>
        )}
      ...
      {(() => {
        const blogPosts = getPostsForGrade(grade, "maths");
      ...
      <section className="bg-white py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <h2 className="text-2xl font-bold text-brand-navy mb-4">
            About Grade {grade} Maths Practice
          </h2>
          <p className="text-gray-600 mb-4">
            Our Grade {grade} Maths quizzes are designed for students aged {info.ageRange}. Each week features multiple-choice questions covering essential maths topics including {info.topics.toLowerCase()}.
          </p>
          <p className="text-gray-600 mb-4">
          ...
          <p className="text-gray-600">
            These quizzes are aligned with the CBSE and ICSE curriculum and help students build problem-solving skills, improve accuracy, and prepare for school examinations.
          </p>
        </div>
      </section>
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntity: [
              {
                "@type": "Question",
                name: `How many Maths quiz questions are available for Grade ${grade}?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text: `There are ${totalQ} Maths quiz questions available for Grade ${grade}, spread across ${weeks.length} weekly quizzes.`,
                },
              },
              {
                "@type": "Question",
                name: `What topics are covered in Grade ${grade} Maths quizzes?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text: `Grade ${grade} Maths quizzes cover ${info.topics}.`,
                },
              },
            ],
          }),
