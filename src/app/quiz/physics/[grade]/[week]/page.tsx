...
import { PHYSICS_GRADES, PHYSICS_GRADE_INFO, getPhysicsGradeWeeks, getPhysicsWeek, getAllPhysicsGradeWeekPairs } from "@/lib/physics-quiz-data";
...
export async function generateStaticParams() {
  return getAllPhysicsGradeWeekPairs().map((p) => ({
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const weekData = getPhysicsWeek(grade, weekNum);

  if (!weekData) return { title: "Quiz Not Found" };

  const topic = weekData.topic || "Physics Practice";
  const enrichment = getQuizEnrichment("physics", grade, weekNum);
  const metaDesc = enrichment
    ...
    : `Take this free Grade ${grade} Physics quiz on ${topic}. ${weekData.questions.length} multiple-choice questions with instant scoring. Perfect for students aged ${PHYSICS_GRADE_INFO[grade]?.ageRange}.`;
  return {
    title: `${topic} Quiz – Grade ${grade} Physics Practice`,
    description: metaDesc,
    keywords: [
      `grade ${grade} physics quiz week ${weekNum}`,
      `${topic.toLowerCase()} quiz`,
      `class ${grade} physics practice`,
      `physics ${topic.toLowerCase()} questions`,
    ],
    alternates: { canonical: `https://www.thepractiseground.in/quiz/physics/${grade}/${weekNum}` },
  };
}

export default async function PhysicsQuizPage({ params }: Props) {
  const { grade: g, week: w } = await params;
  const grade = parseInt(g);
  const weekNum = parseInt(w);
  ...
  const allWeeks = getPhysicsGradeWeeks(grade);
    ...
    <div className="bg-theme-gradient min-h-screen pb-16">
      ...
      <section className="relative overflow-hidden py-8">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6">
          <nav className="text-white/70 text-sm mb-3">
            ...
            <span className="mx-2">/</span>
            <Link href={`/quiz/physics/${grade}`} className="hover:text-white">Grade {grade}</Link>
            <span className="mx-2">/</span>
            <span className="text-white">Week {weekNum}</span>
          </nav>
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">{topic}</h1>
          <p className="text-white/80">
            Physics &middot; Grade {grade} &middot; Week {weekNum} &middot; {weekData.questions.length} questions
          </p>
        </div>
      </section>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-200">
          {prevWeek ? (
            <Link href={`/quiz/physics/${grade}/${prevWeek.week}`} className="text-brand-navy hover:text-emerald-600 transition-colors flex items-center gap-2">
          ...
          {nextWeek ? (
            <Link href={`/quiz/physics/${grade}/${nextWeek.week}`} className="text-brand-navy hover:text-emerald-600 transition-colors flex items-center gap-2">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "Quiz",
            name: `Grade ${grade} Physics Quiz - Week ${weekNum}: ${topic}`,
            about: {
              "@type": "Thing",
              name: topic,
            },
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 3, name: `Grade ${grade}`, item: `https://www.thepractiseground.in/quiz/physics/${grade}` },
              { "@type": "ListItem", position: 4, name: `Week ${weekNum}`, item: `https://www.thepractiseground.in/quiz/physics/${grade}/${weekNum}` },
            ],
          }),
        }}
      />
