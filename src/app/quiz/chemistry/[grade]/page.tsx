...
export async function generateStaticParams() {
  return CHEMISTRY_GRADES.map((g) => ({ grade: String(g) }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Grade ${grade} Chemistry Quiz - ${totalQ}+ Free Questions`,
    description: `Free Chemistry practice quizzes for Grade ${grade} students. ${weeks.length} weekly quizzes with ${totalQ}+ questions covering ${CHEMISTRY_GRADE_INFO[grade]?.topics || "key chemistry topics"}.`,
    keywords: [
      `grade ${grade} chemistry quiz`, `class ${grade} chemistry questions`,
      `chemistry practice grade ${grade}`, `CBSE chemistry class ${grade}`,
      `chemistry MCQ grade ${grade}`, `free chemistry quiz class ${grade}`,
    ],
    alternates: { canonical: `https://www.thepractiseground.in/quiz/chemistry/${grade}` },
  };
}

export default async function ChemistryGradePage({ params }: Props) {
  const { grade: gradeStr } = await params;
  const grade = parseInt(gradeStr);
  ...
  return (
    <div className="bg-theme-gradient min-h-screen">
      ...
      <section className="relative overflow-hidden py-12 sm:py-16">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <WhatsAppShareGrade
            ...
            url={`https://www.thepractiseground.in/quiz/chemistry/${grade}`}
          />
        </div>
      </section>
        ...
        {weeks.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {weeks.map((w) => (
              <Link
                key={w.week}
                href={`/quiz/chemistry/${grade}/${w.week}`}
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 max-w-md mx-auto">
              We don&apos;t have Grade {grade} Chemistry quizzes yet. Browse our available grades below!
            </p>
            <Link href="/quiz/chemistry" className="inline-block mt-6 bg-emerald-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-emerald-600 transition-colors">
      ...
      {(() => {
        const blogPosts = getPostsForGrade(grade, "chemistry");
      ...
      <section className="bg-white py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <h2 className="text-2xl font-bold text-brand-navy mb-4">
            About Grade {grade} Chemistry Practice
          </h2>
          <p className="text-gray-600 mb-4">
            Our Grade {grade} Chemistry quizzes are designed for students aged {info.ageRange}. Each week features multiple-choice questions covering essential chemistry topics including {info.topics.toLowerCase()}.
          </p>
          <p className="text-gray-600 mb-4">
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntity: [
              {
                "@type": "Question",
                name: `How many Chemistry quiz questions are available for Grade ${grade}?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text: `There are ${totalQ} Chemistry quiz questions available for Grade ${grade}, spread across ${weeks.length} weekly quizzes.`,
                },
              },
              {
                "@type": "Question",
                name: `What topics are covered in Grade ${grade} Chemistry quizzes?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text: `Grade ${grade} Chemistry quizzes cover ${info.topics}.`,
                },
              },
            ],
          }),
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { "@type": "ListItem", position: 3, name: `Grade ${grade} Chemistry`, item: `https://www.thepractiseground.in/quiz/chemistry/${grade}` },
            ],
          }),
        }}
      />
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            "@context": "https://schema.org",
            "@type": "Course",
            name: `Grade ${grade} Chemistry Quizzes`,
            description: `Free Chemistry practice quizzes for Grade ${grade} students. ${weeks.length} weekly quizzes with ${totalQ}+ questions covering ${info.topics.toLowerCase()}.`,
