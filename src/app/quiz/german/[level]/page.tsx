...
export async function generateStaticParams() {
  return GERMAN_CEFR_LEVELS.map((l) => ({ level: l }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { level } = await params;
  const info = GERMAN_LEVEL_INFO[level.toLowerCase()];
...
export default async function GermanCefrLevelPage({ params }: Props) {
  ...
  const otherLevels = GERMAN_CEFR_LEVELS.filter((l) => l !== level);
    ...
    <div className="bg-theme-gradient min-h-screen">
      ...
      <section className="relative overflow-hidden py-12 sm:py-16">
        <HeroImage
          ...
          alt="German CEFR grammar quiz practice"
          overlay={0.6}
        />
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 pb-8 space-y-4">
        {otherLevels.map((otherLevel) => {
          const otherInfo = GERMAN_LEVEL_INFO[otherLevel];
          return (
      ...
      <section className="bg-white py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <h2 className="text-2xl font-bold text-brand-navy mb-4">
            About CEFR {info.label} ({info.fullName}) German Practice
          </h2>
          <p className="text-gray-600 mb-4">
            Our {info.label} {info.fullName} German quizzes follow the Common European Framework of Reference for Languages (CEFR), the international standard for measuring language proficiency. Each week features 25 multiple-choice questions focusing on a specific grammar topic at the {info.label} level.
          </p>
          <p className="text-gray-600 mb-4">
          ...
          <p className="text-gray-600">
            These quizzes are suitable for students preparing for Goethe-Zertifikat, TestDaF, or anyone wanting to improve their German grammar at the {info.fullName.toLowerCase()} level.
          </p>
        </div>
      </section>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "Course",
            name: `CEFR ${info.label} (${info.fullName}) German Quizzes`,
            description: `Free ${info.label} ${info.fullName} German grammar quizzes. ${weeks.length} weekly quizzes with ${totalQ}+ questions covering ${info.skills.toLowerCase()}.`,
